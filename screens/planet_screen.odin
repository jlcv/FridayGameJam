...
flock_a : [dynamic]Boid
flock_b : [dynamic]Boid

init_planet_screen :: proc() {
    flock_a := init_boids()
    flock_b := init_boids()
}

draw_planet_screen :: proc(textures: Textures, game: ^Game) {
    fps := rl.GetFPS()
    ...
    if rl.GuiButton(btn_bounds, "Click Me") {
        flock_a = init_boids()
        game.current_screen = managers.GameScreen.MAP
    }
